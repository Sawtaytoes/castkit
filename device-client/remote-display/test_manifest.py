...
class ManifestTests(unittest.TestCase):
    def document(self, **changes):
        return {'version': 1, 'viewport': {'width': 480, 'height': 320}, 'page_url': '/panel',
                'cache': [{'id': 'loading', 'url': '/panel/loading', 'on_tap': '[data-loading]'}], **changes}
    ...
    def test_rejects_cross_origin_urls_unsupported_versions_and_unsafe_limits(self):
        for changes in ({'version': 2}, {'page_url': 'https://other.example/'}, {'page_url': 'https://user:password@example.com/'},
                        {'viewport': {'width': 800, 'height': 480}}, {'refresh': {'heartbeat_ms': 8000}},
                        {'input': {'target_attribute': 'onclick'}}, {'input': {'max_frame_age_ms': 30000}}):
    ...
    def test_no_cache_is_valid_but_extra_cache_slots_are_not_silently_ignored(self):
        self.assertIsNone(parse_manifest(self.document(cache=[]), 'https://example.com/manifest.json')['cache_url'])
