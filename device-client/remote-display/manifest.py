...
def same_origin_url(base, relative):
    ...
    if actual.scheme not in ('http', 'https') or actual.username or actual.password or (actual.scheme, actual.netloc) != (expected.scheme, expected.netloc):
...
def parse_manifest(document, url):
    ...
    if not isinstance(fps, (int,float)) or not 1 <= fps <= 20 or not isinstance(heartbeat, (int,float)) or not 500 <= heartbeat <= 3000:
    ...
    if cached is not None and (not isinstance(cached, dict) or not isinstance(cached.get('id'), str) or not cached['id'] or not isinstance(cached.get('on_tap'), str) or not cached['on_tap']):
