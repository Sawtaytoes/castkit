...
class PreviewRouteTests(AioHTTPTestCase):
    ...
    async def test_the_stream_sends_each_new_frame_and_counts_its_viewer(self):
        ...
        self.assertEqual(
            Image.open(io.BytesIO(first)).convert("RGB").getpixel((1, 1))[0], 254, "red frame"
        )
        ...
        self.assertEqual(
            Image.open(io.BytesIO(second)).convert("RGB").getpixel((1, 1))[2], 254, "blue frame"
        )

        response.close()
        await asyncio.sleep(0.1)
        self.assertEqual(
            (await (await self.client.get("/healthz")).json())["viewers"],
            0,
