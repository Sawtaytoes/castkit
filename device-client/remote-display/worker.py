...
class DisplaySession:
    def __init__(self, config, page, client, services, stop, preview):
        ...
        self.target_options = {'attribute': self.target_attribute, 'loadingSelector': config.get('loading_selector')}
    ...
    async def cancel_contact(self):
        if self.contact is not None:
            await self.cdp.send('Input.dispatchTouchEvent', {'type': 'touchCancel', 'touchPoints': []})
        self.contact = None
    ...
    async def input_loop(self):
        while True:
            ...
            if phase == 0:
                await self.cancel_contact()
                current = await self.page.evaluate(HIT_SCRIPT, {'x': x, 'y': y, 'attribute': self.target_attribute})
            ...
            current = await self.page.evaluate(HIT_SCRIPT, {'x': x, 'y': y, 'attribute': self.target_attribute})
            if current != self.contact['identity'] or time.monotonic() - self.contact['started'] > 5:
                await self.cancel_contact()
                self.processed_touch = sequence
                self.force_frame.set()
                continue
    ...
    async def send_frame(self, payload, touch_id, targets, format_id=2):
        ...
        if format_id == 2:
            rectangles = ';'.join(':'.join(str(round(target[key])) for key in ('x','y','width','height'))
                                  for target in targets if target['loading'])
            await self.client.execute_service(self.services['configure_touch_regions'], {'frame_id': frame_id, 'regions': rectangles})
        future = asyncio.get_running_loop().create_future()
        self.pending[frame_id] = future
        encoded = base64.b64encode(payload).decode('ascii')
        try:
            for offset in range(0, len(encoded), 12000):
                await self.client.execute_service(self.services['frame_chunk'], {
                    'frame_id': frame_id, 'touch_id': touch_id, 'format': format_id,
                    'offset': offset, 'final_chunk': offset + 12000 >= len(encoded),
                    'image_data': encoded[offset:offset + 12000],
                })
            response = await asyncio.wait_for(future, timeout=6)
            if format_id == 2:
                self.guard.remember(frame_id, [Target(**{key: value for key, value in target.items() if key != 'loading'}) for target in targets])
            return response
        finally:
        ...
        try:
            while not self.stop.is_set():
                ...
                if payload != previous_payload or touch_id != previous_touch or self.force_frame.is_set() or cycle - last_sent >= self.config['heartbeat_seconds']:
                    ...
                    previous_payload, last_sent, previous_touch = payload, time.monotonic(), touch_id
                    self.frames += 1
                    if time.monotonic() - self.last_report >= 30:
                        LOG.info('frames=%s payload=%s decode_ms=%.1f draw_ms=%.1f', self.frames, len(payload), int(response[5])/1000, int(response[6])/1000)
                        self.last_report = time.monotonic()
...
async def serve(config):
    ...
    async with async_playwright() as playwright:
        ...
        context = await browser.new_context(viewport={'width': 480, 'height': 320}, device_scale_factor=1, has_touch=True, is_mobile=True, reduced_motion='reduce', accept_downloads=False)
        ...
        config = {**config, **parse_manifest(await manifest_response.json(), config['manifest_url'])}
        page = await context.new_page()
        origin = urlsplit(config['url'])
        async def route_request(route):
            requested = urlsplit(route.request.url)
            if route.request.is_navigation_request() and route.request.frame == page.main_frame and (requested.scheme,requested.netloc) != (origin.scheme,origin.netloc):
                await route.abort()
            else:
        ...
        if config['cache_url']:
            ...
            response = await loading.goto(config['cache_url'], wait_until='networkidle', timeout=15000)
