...
class BrowserTouchTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.context = await self.browser.new_context(viewport={'width': 480, 'height': 320}, has_touch=True)
    ...
    async def event(self, sequence, phase, frame=42):
        await self.session.touches.put(['touch', str(sequence), str(phase), '30', '25', '0', str(frame), '0'])
    ...
    async def test_old_frame_cannot_activate_new_control_at_same_position(self):
        ...
        await self.page.locator('button').evaluate("element => element.onclick = () => element.textContent = 'REMOVED'")
    ...
    async def test_disabled_or_changed_control_during_contact_cancels_click(self):
        await self.event(1, 0)
        await self.page.locator('button').evaluate("element => element.dataset.castkitTarget = 'slot:replacement'")
        await self.event(2, 2)
        self.assertEqual(await self.page.locator('button').inner_text(), 'Slot 1')
        await self.page.locator('button').evaluate("element => { element.dataset.castkitTarget = 'slot:one'; element.disabled = true }")
        await self.event(3, 0)
        await self.event(4, 2)
        self.assertEqual(await self.page.locator('button').inner_text(), 'Slot 1')
    ...
    async def test_cache_frame_is_acknowledged_but_never_becomes_a_touch_target(self):
        ...
        async def acknowledge(service, values):
            if values['final_chunk']:
                self.session.pending[values['frame_id']].set_result(['frame', str(values['frame_id'])])
