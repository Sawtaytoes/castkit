...
class CastKitDisplay : public Component {
  struct FrameJob { int frame_id; int touch_id; int format; std::string encoded; };
 public:
  void setup() override {
    jobs_ = xQueueCreate(1, sizeof(FrameJob *));
    if (!jobs_ || xTaskCreatePinnedToCore(worker, "bitmap_decode", 32768, this, 1, nullptr, 0) != pdPASS) {
      this->mark_failed();
    }
  }
  ...
  void frame_chunk(int frame_id, int touch_id, int format, int offset, bool final_chunk, const std::string &encoded) {
    if (this->is_failed() || busy_) { events_->publish_state("error,receiver_busy"); return; }
    if (offset == 0) { encoded_.clear(); receiving_frame_ = frame_id; }
    if (receiving_frame_ != frame_id || offset != int(encoded_.size()) || encoded_.size() + encoded.size() > 160000) {
      events_->publish_state("error,chunk_order_or_size"); encoded_.clear(); return;
    }
    encoded_.append(encoded);
    if (!final_chunk) return;
    auto *job = new FrameJob{frame_id, touch_id, format, std::move(encoded_)};
    busy_ = true;
    if (xQueueSend(jobs_, &job, 0) != pdTRUE) {
      delete job; busy_ = false; events_->publish_state("error,queue_full");
    }
  }
  void decode_frame(const FrameJob &job) {
    const auto &encoded = job.encoded;
    const int frame_id = job.frame_id, touch_id = job.touch_id, format = job.format;
    const uint32_t started = micros();
    // Decode compressed input in internal RAM to avoid byte-at-a-time PSRAM accesses.
    const size_t capacity = encoded.size() * 3 / 4 + 4;
    using HeapBytes = std::unique_ptr<uint8_t, decltype(&heap_caps_free)>;
    HeapBytes bytes(static_cast<uint8_t *>(heap_caps_malloc(capacity, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)), heap_caps_free);
    HeapBytes input(static_cast<uint8_t *>(heap_caps_malloc(encoded.size(), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)), heap_caps_free);
    if (!bytes || !input) { publish_error("error,compressed_buffer_memory"); return; }
    memcpy(input.get(), encoded.data(), encoded.size());
    size_t count = 0;
    if (mbedtls_base64_decode(bytes.get(), capacity, &count, input.get(), encoded.size()) != 0) {
    ...
    if (format == 2 || format == 3) {
      ...
      if (!destination || tinfl_decompress_mem_to_mem(destination, 480 * 320 * 2, bytes.get(), count,
      ...
      const int consumed = began ? png_.feed_data(bytes.get(), count) : -1;
      ...
      if (!jpeg_pixels_ || !jpeg_decoder_.openRAM(bytes.get(), count, jpeg_draw)) {
