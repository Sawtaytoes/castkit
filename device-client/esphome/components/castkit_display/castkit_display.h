...
class CastKitDisplay : public Component {
 ...
 public:
  void setup() override {
    encoded_ = static_cast<char *>(
        heap_caps_malloc(ENCODED_CAPACITY, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    payload_ = static_cast<uint8_t *>(
        heap_caps_malloc(PAYLOAD_CAPACITY, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  ...
  void configure_touch_regions(int frame_id, const std::string &regions) {
    ...
    while (offset < regions.size() && pending_regions_.size() < 32) {
      Region region;
      if (sscanf(regions.c_str() + offset, "%d:%d:%d:%d", &region.x, &region.y, &region.width,
                 &region.height) != 4 ||
  ...
  void loop() override {
    if (last_frame_ms_ && !is_offline_ && millis() - last_frame_ms_ > 7000) {
      ...
      for (size_t index = 0; index < border.size(); index += 2) {
        border[index] = 0xF8;
        border[index + 1] = 0;
      }
      for (int row = 0; row < 4; row++) {
        display_->draw_pixels_at(0, row, 480, 1, border.data(), display::COLOR_ORDER_RGB,
                                 display::COLOR_BITNESS_565, true, 0, 0, 0);
        display_->draw_pixels_at(0, 319 - row, 480, 1, border.data(), display::COLOR_ORDER_RGB,
                                 display::COLOR_BITNESS_565, true, 0, 0, 0);
      }
    }
  }
  ...
  */
  void frame_chunk(int frame_id, int touch_id, int format, int offset, bool final_chunk,
    ...
    if (this->is_failed() || busy_) {
      events_->publish_state("error,receiver_busy");
      return;
    }
    if (offset == 0) {
      encoded_size_ = 0;
    ...
    if (receiving_frame_ != frame_id || offset != int(encoded_size_) ||
        encoded_size_ + encoded.size() > ENCODED_CAPACITY) {
      events_->publish_state("error,chunk_order_or_size");
      encoded_size_ = 0;
      return;
    }
    ...
    if (xQueueSend(jobs_, &job, 0) != pdTRUE) {
      busy_ = false;
      events_->publish_state("error,queue_full");
    }
  }
  void decode_frame(const FrameJob &job) {
    ...
    if (mbedtls_base64_decode(payload_, PAYLOAD_CAPACITY, &count,
      ...
      publish_error("error,invalid_base64");
      return;
    }
    ...
    if (format == 2 || format == 3) {
      ...
      if (!destination) {
        destination = static_cast<uint8_t *>(
            heap_caps_malloc(480 * 320 * 2, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
      }
      if (!destination ||
          tinfl_decompress_mem_to_mem(destination, 480 * 320 * 2, payload_, count,
                                      TINFL_FLAG_PARSE_ZLIB_HEADER) != 480 * 320 * 2) {
        publish_error("error,rgb565_inflate");
        return;
      }
      pixels = destination;
    } else if (format == 1) {
      ...
      if (consumed < 0 || !finished || png_.get_width() != 480 || png_.get_height() != 320) {
        publish_error("error,png_decode");
        return;
      }
      ...
      png_.release();
      if (!jpeg_pixels_) {
        jpeg_pixels_ = static_cast<uint8_t *>(
            heap_caps_malloc(480 * 320 * 2, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
      }
      if (!jpeg_pixels_ || !jpeg_decoder_.openRAM(payload_, count, jpeg_draw)) {
        publish_error("error,jpeg_open");
        return;
      }
      if (jpeg_decoder_.getWidth() != 480 || jpeg_decoder_.getHeight() != 320 ||
          jpeg_decoder_.getJPEGType() == JPEG_MODE_PROGRESSIVE) {
        jpeg_decoder_.close();
        publish_error("error,jpeg_dimensions");
        return;
      }
      ...
      jpeg_decoder_.close();
      if (!decoded_ok) {
        publish_error("error,jpeg_decode");
        return;
      }
      pixels = jpeg_pixels_;
    }
    ...
    this->defer(
        [this, frame_id, touch_id, format, count, started, decoded_base64, decoded, pixels]() {
          ...
          const bool should_draw =
              format != 3 && (optimistic_touch_id_ == 0 || touch_id >= optimistic_touch_id_);
          if (should_draw) {
            display_->draw_pixels_at(0, 0, 480, 320, pixels, display::COLOR_ORDER_RGB,
                                     display::COLOR_BITNESS_565, true, 0, 0, 0);
            ...
            optimistic_touch_id_ = 0;
            active_regions_ =
                pending_region_frame_ == frame_id ? pending_regions_ : std::vector<Region>{};
          }
          const uint32_t drawn = micros();
          char message[240];
          const int touch_latency =
              touch_id == last_touch_id_ && touch_id > 0 ? int(millis() - last_touch_ms_) : -1;
          snprintf(message, sizeof(message), "frame,%d,%d,%zu,%u,%u,%u,%d,%u,%u", frame_id,
                   touch_id, count, unsigned(decoded_base64 - started),
                   unsigned(decoded - decoded_base64), unsigned(drawn - draw_started),
                   touch_latency, unsigned(heap_caps_get_free_size(MALLOC_CAP_INTERNAL)),
  ...
  void touch(int phase, int x, int y) {
    ...
    if (phase == 0) {
      ...
      for (const auto &region : active_regions_) {
        if (x >= region.x && x < region.x + region.width && y >= region.y &&
          ...
          is_loading_tap_ = true;
        }
      }
    }
    if (is_contact_ignored_) {
      return;
    }
    last_touch_id_++;
    last_touch_ms_ = millis();
    if (phase == 1 && (std::abs(x - start_x_) > 10 || std::abs(y - start_y_) > 10)) {
      is_loading_tap_ = false;
    }
    ...
    if (phase != 2) {
      ...
      for (int row = -6; row <= 6; row++) {
        int half_width = 0;
        while ((half_width + 1) * (half_width + 1) + row * row <= 36) {
          half_width++;
        }
        const int top = y + row;
        const int start = std::max(0, x - half_width);
        const int end = std::min(479, x + half_width);
        if (top >= 0 && top < 320 && end >= start) {
          display_->draw_pixels_at(start, top, end - start + 1, 1, marker.data(),
    ...
    if (phase == 2 && is_loading_tap_ && loading_pixels_) {
      ...
      display_->draw_pixels_at(0, 0, 480, 320, loading_pixels_, display::COLOR_ORDER_RGB,
                               display::COLOR_BITNESS_565, true, 0, 0, 0);
      is_loading_tap_ = false;
    }
    char message[180];
 ...
 protected:
  ...
  static int jpeg_draw(JPEGDRAW *draw) {
    ...
    if (draw->x < 0 || draw->y < 0 || draw->x + draw->iWidth > 480 ||
        draw->y + draw->iHeight > 320) {
      return 0;
    }
  ...
  runtime_image::RuntimeImage png_{runtime_image::PNG, image::IMAGE_TYPE_RGB565,
                                   image::TRANSPARENCY_OPAQUE, nullptr, true};
  int shown_frame_{0};
