...
| Panel | photo_people |
| --- | --- |
| `eink-07769e` (13.3" #1) | `Rowan, Callum, Elliot, Morgan, Alex` |
| `eink-4da1be` (13.3" #2) | `Rowan, Callum, Elliot, Wren, Jesse` |

(#1→Alex+Morgan's family, #2→Wren+Jesse's family. If the physical frames are
...
Evidence:
- `castkit/eink-6e6697/photo_people` = `Rowan, Callum, Elliot` (22 B) → renders,
...
trigger that re-asserts `Rowan, Callum, Elliot` whenever the people text goes
...
- **MQTT** (retained, persists across restarts):
  ...
  - broker `mqtts://mqtt.octen.dev:8883`, user `inkcast` (creds in the CastKit
    ...
    comma-separated list of Immich person names, e.g. `Rowan, Callum, Elliot`.
...
| `eink-6e6697` kitchen 7.3" | Photo Frame | `Rowan, Callum, Elliot` | off | 0 | ✅ cycling |
