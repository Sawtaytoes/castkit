...
- **Supersedes:**
  ...
  [2026-07-04 The now-playing source is an HA config entity: a per-device priority-ordered media_player list](2026-07-04-now-playing-source-is-ha-config-priority-list.md)
- **Superseded by:** —
...
Assistant's job. The `2026-07-04` now-playing-source priority-list feature made
it worse — it moved *config* to MQTT (good) but added *priority-resolution
logic* into the renderer (more HA-domain knowledge, wrong direction).
...
- **Supersedes** the three prior decisions above (now-playing reads HA, agenda
  pulls from HA, and the now-playing source priority list). HA — via a
  template/automation — now *publishes* each view's payload to Inkcast's MQTT
  topic rather than Inkcast reading HA, and the now-playing priority logic moves
  out of Inkcast into an HA template. (The `home-displays` `ma-nowplaying-bridge`
  is still correctly gone; this is not that bridge — HA's own automations
  publish straight to Inkcast's MQTT, no separate service.)
- **Discards the `feat/now-playing-source-ha-config` source-picker** (not
  merged) — its priority logic relocates to an HA template.
